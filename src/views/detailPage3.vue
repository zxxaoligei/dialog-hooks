<template>
    <div class="contianer">
        detailPage333333333333333333333
        <el-button type="success" @click="openDialog">打开第四个弹框</el-button>
...
const openDialog = () => {
    addDialog({
        topTagName: '第四个明细内容',
        iconName: "form",
        component: () => import("@/views/detailPage4.vue"),
        data: {
        },
        collbackFn: () => { //弹框内部组件 保存成功后的回调函数
            console.log("当弹窗任务结束后，调用父页面的回掉函数。（比如我新增完成了需要刷新列表页面）--detailPage3");
        }
    })
}
